...
export default function BinanceLive() {
  ...
  const handleStart = async (config: any) => {
    ...
    try {
      await fetch(`${API}/api/forward-test/start`, {
        ...
        body: JSON.stringify({ ...config, exchange: 'binance_live' }),
      });
      await fetchStatus();
    } finally {
      setLoading(false);
  ...
  const handleManualClose = async (symbol: string, type: string, qty: number) => {
    ...
    await fetch(`${API}/api/binance/close-manual`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ symbol, type, quantity: qty, isLive: true }) });
    await fetchAccount();
  };
  ...
  return (
    ...
    <div style={{ display: 'flex', gap: sidebarMode === 'none' ? '0' : '1rem', flex: 1, overflow: 'hidden', position: 'relative' }}>

...
export default function PositionChartModal({ 
  ...
  const { data, loading } = useBinanceData(symbol, interval);
  const indicators = useIndicators(data);

  const entryUnix = typeof entryTime === 'string' ? Math.floor(new Date(entryTime).getTime() / 1000) : Math.floor(entryTime / 1000);
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10000, padding: '20px' }}>
      <div className="glass-panel" style={{ 
        width: '100%', maxWidth: '1050px', maxHeight: 'calc(100vh - 60px)',
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '1.5rem', display: 'flex', flexDirection: 'column', position: 'relative', gap: '1rem', background: 'linear-gradient(to bottom, #131722, #0b0e14)' }}>
           {loading && (
           ...
           <div style={{ height: showRSI ? '400px' : '550px', flexShrink: 0, transition: 'height 0.3s ease' }}>
              <PriceChart 

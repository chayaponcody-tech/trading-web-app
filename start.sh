#!/bin/bash
# start.sh — Self-healing startup for Linux (Ubuntu)
# Usage: ./start.sh
...
setup_python_env() {
    ...
    echo -e "\033[36m[System] Checking Python environment for $service_name...\033[0m"
    cd "$service_path"
    
    # Check for venv
    if [ ! -d "venv" ]; then
        echo -e "\033[33m🐍 Creating virtual environment for $service_name...\033[0m"
        python3 -m venv venv
    fi
    
    # Use venv python
    local P_EXE="./venv/bin/python3"
    if [ ! -f "$P_EXE" ]; then P_EXE="./venv/bin/python"; fi
    
    # Verify uvicorn exists in venv, otherwise install requirements
    if ! $P_EXE -m uvicorn --version >/dev/null 2>&1; then
        echo -e "\033[33m📥 Installing Python requirements for $service_name...\033[0m"
...
# Use global python3 for Poly (or we could venv it too)
...
echo "  Strategy AI          : http://[YOUR-SERVER-IP]:8000"
echo "  Quant Engine         : http://[YOUR-SERVER-IP]:8002"
echo "  Polymarket Dashboard : http://[YOUR-SERVER-IP]:8080"
echo ""

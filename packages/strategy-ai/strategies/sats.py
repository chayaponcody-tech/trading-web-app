...
class SelfAwareTrendSystem(BaseStrategy):
    """
    Self-Aware Trend System (SATS) v2.1.0
    Adaptive SuperTrend with Character-Flip (Pre-emptive Exit). 
    Includes TQI (ER, Vol, Struct, Mom, ADX).
    """
    ...
    def populate_signals(self, df: pd.DataFrame, params: dict) -> pd.DataFrame:
        ...
        for i in range(1, len(df)):
            ...
            if curr_trend == 1 and prev_trend == -1 and curr_tqi >= tqi_min_entry:
                ...
                # Pre-emptive exit logic if already in position (handled by signal engine usually, 
                # but here we set signal to NONE or potentially an EXIT signal if needed)
                pass
    ...
    def get_metadata(self):
        return {
            "name": "Self-Aware Trend System (SATS)",
